...
class GlashWindow(QMainWindow):
    ...
    def _print_page(self):
        ...
        try:
            ...
            # Fallback: save as PDF on Desktop
            self._save_as_pdf()
    ...
    def _save_as_pdf(self):
        ...
        if not v:
            return
        # Use Downloads folder — exists on both Windows and macOS
        dl_dir = os.path.expanduser("~/Downloads")
        os.makedirs(dl_dir, exist_ok=True)
        safe_title = "".join(c for c in v.title()[:30] if c.isalnum() or c in " _-")
        path = os.path.join(dl_dir,
            f"Glash_{safe_title}_{int(datetime.now().timestamp())}.pdf"
        )
    ...
    def _save_page(self):
        ...
        if v:
            ...
            os.makedirs(dl_dir, exist_ok=True)
            path = os.path.join(dl_dir,
                f"Glash_Page_{int(datetime.now().timestamp())}.html"
            )
...
# ─────────────────────────────────────────────
def main():
    # Windows: high-DPI and GPU flags
    if sys.platform == "win32":
        os.environ.setdefault("QTWEBENGINE_CHROMIUM_FLAGS",
            "--enable-features=NetworkServiceInProcess "
            "--enable-smooth-scrolling "
            "--disable-gpu-sandbox")
        # Tell Qt to render the menu bar inside the window (not native Windows style)
    ...
    # setDesktopFileName only exists on Linux; skip on Windows/macOS
